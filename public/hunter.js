...
$('body').mousemove(function(event){
  ...
  $('.flashcursor').css({
    'position':'absolute',
    'top': event.clientY + offsety + 5,
    'left':event.clientX + offsetx + 5,
    'z-index': '3'
  })
})
  ...
  if(lampDebounce == false){ //if the  the debounce gate is open
   ...
   if(lampDebounce == false){ //if the  the debounce gate is open
    if( ghostOverlapBounds2 == 1 ){ //we hit!
       $('.ghost').attr('src', 'image/ghostbounds2.gif') //change the image
   ...
   if(lampDebounce == false){ //if the  the debounce gate is open
    if( lightOverlapGhost == 1 ){ //we hit!
       $('.ghost').attr('src', 'image/ghostcaught1.gif').css({
         'visibility':'visible'
       })
